...
| `soil-dry` | 耕地が乾くかどうか |
| `time-lock` | 時間を固定する |
| `chest-access` | チェストを利用できる（アイテムの出し入れができる）かどうか |
||**要注意！**: <br>`allow`にすると、保護のオーナーやメンバー以外もチェストを利用可能になります。<br>`deny`にすると、保護のオーナーやメンバーを含め、誰もチェストを利用できません。<br>何も値を指定しない（フラグが設定されていない）と、保護のオーナーとメンバーだけがチェストを利用可能になります。（デフォルトの挙動です。）|
